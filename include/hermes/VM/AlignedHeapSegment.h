...
class AlignedHeapSegment {
 ...
 public:
  ...
  class Contents {
   ...
   private:
    ...
    Contents(size_t segmentSize) {
      ...
      if (segmentSize == kSegmentUnitSize) {
        ...
        cards_ = (new AtomicIfConcurrentGC<CardStatus>[cardTableSize] {});
      }
    }

    ~Contents() {
      if (cards_ != inlineCardsArray_) {
        delete[] cards_;
      }
    }
    ...
    union {
      ...
      /// SHSegmentInfo struct and the outline cards_ pointer.
      struct {
   ...
   public:
    ...
    static constexpr size_t kFirstUsedIndex =
        (sizeof(SHSegmentInfo) + sizeof(cards_));
  ...
  size_t getEndCardIndex() {
    return contents()->getEndIndex();
  }
  ...
  static void dirtyCardForAddressInLargeObj(
    ...
    segContents->cards_[index].store(
  ...
  static bool isCardForAddressDirty(const GCCell *owningObj, const void *addr) {
    ...
    return segContents->cards_[index].load(std::memory_order_relaxed) ==
        Contents::CardStatus::Dirty;
  }
  ...
  bool isCardForIndexDirty(size_t index) {
    return contents()->cards_[index].load(std::memory_order_relaxed) ==
        Contents::CardStatus::Dirty;
  }
  ...
  void clearCardStatus() const {
    cleanOrDirtyRange(
        contents(),
        Contents::kFirstUsedIndex,
        contents()->getEndIndex(),
 ...
 protected:
  ...
  static void cleanOrDirtyRange(
    ...
    for (size_t index = from; index < to; index++) {
      segContents->cards_[index].store(cleanOrDirty, std::memory_order_relaxed);
    }
  }
  ...
  OptValue<size_t> findNextCardWithStatus(
    ...
    for (size_t idx = fromIndex; idx < endIndex; idx++)
      if (contents()->cards_[idx].load(std::memory_order_relaxed) == status)
        return idx;
